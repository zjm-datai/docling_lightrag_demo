...
- **sra_rag/__init__.py** - 模块导出文件
  - 导出主要类：DoclingParser, LightRAGIndexer, LightRAGRetriever, RAGConfig
  - 包含模块文档说明

- **sra_rag/config.py** - 配置管理模块
  - RAGConfig 数据类，集中管理所有配置
  - 默认配置实例
  - 支持字典序列化和反序列化
...
- **sra_rag/indexer/lightrag_indexer.py** - LightRAG 索引器实现
  - 集成 LightRAG 双级检索系统
  - 配置 Qwen3-30B-A3B-GPTQ-Int4 LLM
  - 配置 bge-m3 Embedding 模型（1024维度）
  - 异步 Embedding 生成
  - 文档索引和查询功能
  - 完整的错误处理和日志记录
...
- **LLM API**: http://211.90.240.240:30001/v1
- **Embedding**: bge-m3 (1024 维度)
- **分块方式**: Docling HybridChunker
